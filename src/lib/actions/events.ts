...
export async function getActiveEvent() {
  try {
    const event = await db.select().from(events).where(eq(events.isFeatured, true)).limit(1);
    if (event.length === 0) {
...
export async function registerEvent(eventId: string, data: Record<string, any>) {
  try {
...
export async function getEventRegistrants(slug: string) {
  try {
    ...
    if (event.length === 0) {
      console.log("[DEBUG getEventRegistrants] Event not found for slug:", slug);
      return { success: false, error: "Event not found" };
    }
    ...
    return { success: true, data: { event: event[0], registrants: eventRegistrations } };
  } catch (error) {
