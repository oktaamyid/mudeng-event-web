      ...
      <div className="container">
        <div className="curriculum__header">
          <p className="curriculum__subtitle text-subtitle">
            Kuasai berbagai keahlian digital kreatif mulai dari desain antarmuka, pembuatan konten visual, hingga kolaborasi proyek multimedia.
          </p>
          <h2 className="curriculum__heading">What this<br />teaches you</h2>
        </div>

        <div className="curriculum__grid">
          {modules.map((mod, i) => (
            <div className="curriculum-card" key={i}>
              <div className={`curriculum-card__header ${mod.headerClass}`}>
                <div className="curriculum-card__top">
                  <div className={`curriculum-card__icon ${mod.iconClass}`}>
                    {mod.icon}
                  </div>
                  <span className="curriculum-card__number">{mod.number}</span>
                </div>
                <div className="curriculum-card__info">
                  <h3 className="curriculum-card__title">{mod.title}</h3>
                  <p className="curriculum-card__desc">{mod.desc}</p>
                </div>
              </div>
              <div className="curriculum-card__details">
                <div className="curriculum-card__lessons-title">What you'll learn</div>
                <div className="curriculum-card__lessons">
                  {mod.lessons.map((lesson, j) => (
                    <div className="curriculum-card__lesson" key={j}>
                      <span className="curriculum-card__lesson-num">{j + 1}.</span> {lesson}
                    </div>
                  ))}
                </div>
                <div className="curriculum-card__tags-title">Key themes</div>
                <div className="curriculum-card__tags">
                  {mod.tags.map((tag, k) => (
                    <div className="tag-pill" key={k}>
                      <div className="tag-pill__icon" style={{ background: tag.color }}></div> {tag.label}
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="curriculum__banner">
          ...
          <div className="curriculum__banner-text">
            <div className="curriculum__banner-title">Sudah siap tingkatkan skill Kamu?</div>
            <div className="curriculum__banner-desc">
              Pelatihan ini dirancang praktis agar Kamu bisa langsung praktek membuat karya portofolio.
            </div>
          </div>
          <div className="flex items-center justify-center gap-2 md:flex-row">
            <a href="#cta" className="inline-block bg-brand !text-white font-body font-medium text-[16px] py-[13px] px-[28px] rounded-[100px] transition-transform hover:scale-105">Daftar Sekarang</a>
            <a href="#services" className="inline-block bg-brand-light text-text-main font-body font-medium text-[16px] py-[13px] px-[28px] rounded-[100px] transition-transform hover:scale-105">Lihat Detail</a>
          </div>
        </div>
      </div>
    </section>
