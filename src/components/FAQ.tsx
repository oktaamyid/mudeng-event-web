...
const faqItems = [
  ...
  {
    question: 'Program belajar multimedia mana yang paling sesuai untuk saya?',
  ...
  {
    question: 'Apakah ada sesi konsultasi langsung dengan mentor jika saya menemui kesulitan?',
  ...
  {
    question: 'Bagaimana portofolio MUDENG ini dapat membantu saya dalam melamar kerja?',
  ...
  return (
    <section className="section-spacing pb-32" id="faq">
      <div className="container max-w-[700px]">
        <h2 className="heading-h2 mb-10 text-center">Let's clear a few things up</h2>

        <div className="space-y-3 flex flex-col items-center">
          {faqItems.map((item, i) => (
            <div
              key={i}
              className={`w-[560px] max-w-full bg-nav-outer overflow-hidden border border-divider transition-all ${
                activeIndex === i ? 'rounded-[20px]' : 'rounded-[20px]'
              }`}
            >
              <button
                className="w-full min-h-[70px] flex items-center justify-between gap-4 px-6 py-3 text-left focus:outline-none"
                onClick={() => toggleItem(i)}
              >
                <span className="text-[18px] text-text-main pr-2">{item.question}</span>
                <div className={`flex-shrink-0 w-7 h-7 rounded-full bg-brand flex items-center justify-center transition-transform ${activeIndex === i ? 'rotate-45' : ''}`}>
              ...
              >
                <div className="overflow-hidden">
                  <p className="px-6 pb-4 text-[15px] text-text-muted">{item.answer}</p>
                </div>
              </div>
            </div>
          ))}
